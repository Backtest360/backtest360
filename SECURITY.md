...
Security fixes are issued for the **latest released version** of `backtest360`
on PyPI. Please upgrade to the latest version before reporting an issue:

```bash
pip install --upgrade backtest360
```
...
- The affected version (`python -c "import backtest360; print(backtest360.__version__)"`)
...
This repository contains the **API client** only. Reports about the Backtest360
