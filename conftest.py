"""Root pytest configuration for the Backtest360 client.

IMPORT-ISOLATION GUARD
======================
``pythonpath = ["src"]`` in pyproject prepends this project's ``src/`` to
``sys.path`` so the test suite imports the local ``backtest360`` package. If a
different copy of the package is installed elsewhere on the interpreter (for
...
backtest360`` to that other copy — so the tests would exercise code you are not
looking at.
